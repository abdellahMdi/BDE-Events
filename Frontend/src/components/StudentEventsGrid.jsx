...
export default function StudentEventsGrid({ events, onReserve, onCancel, actionLoading }) {
    ...
    return (
      <div className="col-span-full py-16 text-center bg-white rounded-3xl border border-stone-200 shadow-sm space-y-3">
        ...
        <div>
          ...
          <p className="text-xs text-stone-500 mt-1">Revenez plus tard pour voir les nouveaux événements organisés.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
      {events.map((event) => (
        <StudentEventCard
          ...
          onReserve={onReserve}
          onCancel={onCancel}
          isBusy={Boolean(actionLoading[event.id])}
        />
      ))}
    </div>
  );
